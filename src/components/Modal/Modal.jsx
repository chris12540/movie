...
class Modal extends Component {
  ...
  addToList = () => {
    ...
    Axios.post(`/api/lists/${list}`, { listId: list, ...this.props.item }).then(
      (res) => {
        res.status === 201 && this.added();
      }
    );
  };
  ...
  getSelection = (type) => {
    ...
    return type === "value"
      ? lists.options[lists.selectedIndex].value
      : type === "name"
      ? lists.options[lists.selectedIndex].text
      : "";
  };
  ...
  componentDidMount() {
    ...
    Axios.get(
      `https://www.googleapis.com/youtube/v3/search?part=id&q=${query}%20Trailer&maxResults=1&key=${process.env.REACT_APP_KEY}`
    ).then((res) => {
  ...
  render() {
    ...
    const userLists = this.state.userLists.map((list) => {
      return (
        <option key={list.id} name={list.name} value={list.id}>
          {list.name}
        </option>
      );
    });
    return (
      <div className={`modal-container ${id}`}>
        ...
        <div className={showPoster ? "modal-rotate" : "modal-rotate rotate"}>
          <div className="rotate-card" onClick={this.rotateCard}>
            {" "}
            ->{" "}
          </div>
          {poster ? (
          ...
          <div className="modal">
            ...
            {userLists.length ? (
              <div className="add-to-list">
                ...
                <button onClick={this.addToList} className="add">
                  Add
                </button>
              </div>
            ) : (
              <Link to="/login" className="btn">
                Login to add to a list
              </Link>
            )}
          </div>
        </div>
        {added && (
          <div className="added">
            <h1>
              {title} Added To {this.getSelection("name")}
            </h1>
          </div>
        )}
      </div>
